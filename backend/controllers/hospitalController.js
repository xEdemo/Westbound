...
const createHospital = asyncHandler(async (req, res) => {
    const {name, description, location, users, staff, services, hazards} = req.body;
    if (!name) {
...
const deleteHospitalById = asyncHandler(async (req, res) => {
    ...
    const hospital = await Hospital.findById(hospitalId);
    if (!hospital) {
        res.statusres.status(StatusCodes.NOT_FOUND);
        throw new Error(`Hospital with ID ${hospitalId} not found in hospital records`);
    }
