...
const createCompany = asyncHandler(async (req, res) => {
    const {name, description, isCompanyStockCompany, type, image, cost, fixedPositionsAvailable, rewards, ownerId, taxExempt, employees, finances, legalStatus, upgrades} = req.body;
    if (!name) {
...
const updateCompanyById = asyncHandler(async(req, res) => {
    ...
    if (isCompanyStockCompany && isCompanyStockCompany !== hospital.isCompanyStockCompany) {
    ...
    if (fixedPositionsAvailable && fixedPositionsAvailable !== hospital.fixedPositionsAvailable) {
...
const deleteCompanyById = asyncHandler(async (req,res) =>{
    ...
    if (!company) {
        ...
        throw new Error(`Company with Id ${companyId} not found in company records.`);
    }
